...
public class Foop {
	public static void main(String[] args) throws IOException,
		...
		while (i < args.length) {
			...
			else if(args[i].trim() == "--username"){
				i++;
				job.set("foop.ftp.username", args[i].trim());
				i++;
			}else if(args[i].trim() == "--password"){
				i++;
				job.set("foop.ftp.password", args[i].trim());
				i++;
			}else if(args[i].trim() == "--password-file"){
				i++;
				job.set("foop.ftp.password.file", args[i].trim());
				i++;
			}else if(args[i].trim() == "--password-alias"){
				i++;
				job.set("foop.ftp.password.alias", args[i].trim());
				i++;
			}else if(args[i].trim() == "--source-dir"){
				i++;
				job.set("foop.source", args[i].trim());
				i++;
			}else if(args[i].trim() == "--target-dir"){
				i++;
				job.set("foop.target", args[i].trim());
				i++;
			}else if(args[i].trim() == "--ftp-type-ascii"){
				job.set("foop.ftp.type", "ascii");
				i++;
			}else if(args[i].trim() == "--ftp-type-bin-rdw"){
