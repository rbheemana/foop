...
import org.apache.hadoop.fs.ftp.*;
...
public class FTPFileSystem extends FileSystem {
 ...
 public static final String FS_FTP_FILE_TYPE = "fs.ftp.file.type";
 public static final String FS_FTP_GET_RDW = "fs.ftp.get.rdw";
 public static final String FS_FTP_LOCAL_PASSIVE = "fs.ftp.local.passive";
 ...
 private FTPClient connect() throws IOException {
   ...
   if(conf.getBoolean(FS_FTP_LOCAL_PASSIVE, false)){
   ...
   if (!FTPReply.isPositiveCompletion(reply)) {
     ...
     client.setFileTransferMode(conf.getInt(FS_FTP_TRANSFER_MODE, FTP.BLOCK_TRANSFER_MODE));
     client.setFileType(conf.getInt(FS_FTP_FILE_TYPE,FTP.BINARY_FILE_TYPE));
     client.setBufferSize(DEFAULT_BUFFER_SIZE);
     if(conf.getBoolean(FS_FTP_GET_RDW, false)){
...
// check input file path is directory
 ...
 public Path[] listFiles(Path file)
	     throws IOException {
	   if (isFile(file)){
	     throw new FileNotFoundException("File " + file + " is not Directory");
	   }
	   FTPClient client = connect();
	   Path workDir = new Path(client.printWorkingDirectory());
	   Path absolute = makeAbsolute(workDir, file);
       ...
       for (FTPFile ftpFile : ftpFiles) {
    	   if (ftpFile.isDirectory()){
    		   paths.addAll(Arrays.asList(listFiles(new Path(pathName,ftpFile.getName()))));
    	   }
    	   else{
    		   paths.add(new Path(pathName,ftpFile.getName()));
    	   }
	   } 
 ...
 private FileStatus getFileStatus(FTPClient client, Path file)
   ...
   if (ftpFiles != null) {
     ...
     if (fileStat == null) {
       throw new FileNotFoundException("Stat Null: File " + file + " does not exist.");
     }
   } else {
